 ...
 * who do.”
 *
 * File: Implementation file for Library Class
 */
...
RefItem::RefItem(QStringList& plst)
    : m_ItemType(plst.takeFirst()),
      m_ISBN(plst.takeFirst()),
...
QString RefItem::getItemType() const { return m_ItemType; }

QString RefItem::getISBN() const { return m_ISBN; }

QString RefItem::getTitle() const { return m_Title; }

int RefItem::getNumberOfCopies() const { return m_NumberOfCopies; }
...
// end
void RefItem::setNumberOfCopies(int newValue) { m_NumberOfCopies = newValue; }

Book::Book(QString type, QString isbn, QString title, QString author,
           QString pub, int year, int numCopies)
...
Book::Book(QStringList& plst)
    : RefItem(plst),
      m_Author(plst.takeFirst()),
      m_Publisher(plst.takeFirst()),
...
int Dvd::getNumberOfDisks() const { return m_NumberOfDisks; }

bool Dvd::isTwoSided() const { return m_TwoSided; }

Film::Film(QString type, QString isbn, QString title, int disks, bool twoSided,
           QString star, QString director, int minutes, bool blueray,
           int numCopies)
...
QString Film::getStar() const { return m_Star; }

QString Film::getDirector() const { return m_Director; }

int Film::getMinutes() const { return m_Minutes; }

bool Film::isBlueray() const { return m_BlueRay; }

DataDvd::DataDvd(QString type, QString isbn, QString title, int disks,
                 bool twoSided, QString protocol, int numCopies)
    : Dvd(type, isbn, title, disks, twoSided, numCopies),
      m_DBProtocol(protocol) {}

DataDvd::DataDvd(QStringList& plst)
    : Dvd(plst), m_DBProtocol(plst.takeFirst()) {}

QString DataDvd::toString(QString sep) const {
  return QString("%1%2%3").arg(Dvd::toString(sep)).arg(sep).arg(m_DBProtocol);
}

QString DataDvd::getDBProtocol() const { return m_DBProtocol; }
...
Library& Library::operator=(const Library&) { return *this; }

void Library::addRefItem(RefItem*& refitem) { /* Parameter is a pointer
                                                 reference so that null
                                                 assignment after delete is
                                                 possible. */
...
bool Library::isInList(QString isbn) { return findRefItem(isbn); }

QString Library::toString(QString sep) const {
  QStringList reflst;
  for (int i = 0; i < size(); ++i) reflst << at(i)->toString();
